...
function categorizeFile(filePath) {
	...
	if (relative.startsWith('.github/instructions/')) {
		...
		return 'reports';
	} else if (relative === 'AGENTS.md' || relative === 'README.md' || relative === 'CONTRIBUTING.md') {
		return 'root';
	} else {
		return 'other';
	}
...
if (detailed) {
	...
	results
		...
		.forEach(result => {
			...
			const displayPath = relativePath.length > 50
...
Object.entries(categories).forEach(([category, stats]) => {
	log('INFO', `${category}: ${stats.files} files, ${stats.tokens.toLocaleString()} tokens`);
});
