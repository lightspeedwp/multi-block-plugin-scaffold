...
function findTransitiveReferences(graph) {
	...
	for (const node in graph) {
		...
		const transitiveRefs = [...allRefs].filter(ref => !directRefs.has(ref));
...
files.forEach(filePath => {
	try {
		...
		const resolvedRefs = references.map(ref => resolvePath(filePath, ref));
...
const stats = {
	...
	filesWithRefs: Object.values(fileData).filter(d => d.directRefCount > 0).length,
	totalReferences: Object.values(fileData).reduce((sum, d) => sum + d.directRefCount, 0),
...
recommendations
	...
	.slice(0, 10)
	.forEach((rec, i) => {
		const status = rec.recommendation === 'CRITICAL' ? '🔴' :
		               rec.recommendation === 'REDUCE' ? '🟡' :
		               rec.recommendation === 'REVIEW' ? '🟠' : '🟢';
		console.log(`  ${i + 1}. ${status} ${rec.file}`);
		console.log(`     Direct: ${rec.directRefs}, Transitive: ${rec.transitiveRefs}, ${rec.reason}`);
	});
...
if (outputCsv) {
	const csvFile = path.join(__dirname, '../tmp', `frontmatter-audit-${timestamp}.csv`);
	const csvLines = [
...
if (outputGraph) {
	const graphFile = path.join(__dirname, '../tmp', `frontmatter-graph-${timestamp}.json`);
