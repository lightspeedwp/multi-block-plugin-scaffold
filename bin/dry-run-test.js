...
function dryRunLog( level, message ) {
	...
	console.log( `${ colors[ level.toLowerCase() ] || colors.reset }${ entry.trim() }${ colors.reset }` );
}
...
async function main() {
	...
	const commands = args.length > 0 ? args : [ 'lint:js', 'lint:css', 'test:unit' ];
	...
	Object.entries( DRY_RUN_VALUES ).slice( 0, 5 ).forEach( ( [ key, value ] ) => {
		log( `   ${ key }: ${ value }`, 'reset' );
		dryRunLog( 'DEBUG', `Test value: ${ key } = ${ value }` );
	} );
	log( '   ... (see bin/dry-run-config.js for full list)\n', 'reset' );
	...
	try {
		...
		for ( const command of commands ) {
			const npmCommand = command.startsWith( 'npm' ) ? command : `npm run ${ command }`;
