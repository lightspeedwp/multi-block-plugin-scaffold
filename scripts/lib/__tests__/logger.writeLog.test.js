...
const logFile = path.join(logsDir, `generate-theme-${testSlug}.log`);
	...
	test('appends multiple log entries', () => {
		const entry1 = logger.createLogEntry(testSlug, 'started', {}, {}, '/foo');
		const entry2 = logger.createLogEntry(testSlug, 'success', {}, {}, '/bar');
	...
	test('does not throw if log directory is missing', () => {
		...
		const entry = logger.createLogEntry(testSlug, 'started', {}, {}, '/foo');
