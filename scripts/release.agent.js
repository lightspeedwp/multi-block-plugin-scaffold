...
function findPluginFile(rootDir) {
    const phpFiles = fs.readdirSync(rootDir).filter(file => file.endsWith('.php') && file !== 'uninstall.php');
...
function checkVersionConsistency() {
    ...
    try {
        ...
        if (pluginFile) {
            ...
            versions[path.basename(pluginFile)] = extractHeaderVersion(pluginContent);
            versions[`${path.basename(pluginFile)} constant`] = extractConstantVersion(pluginContent);
        } else {
            warning('Plugin main file not found; cannot verify header version');
            addResult('important', 'version', 'Plugin main file missing', 'warn');
        }

        const readmePath = path.join(rootDir, 'readme.txt');
        if (fs.existsSync(readmePath)) {
            const readmeContent = fs.readFileSync(readmePath, 'utf8');
            const stableMatch = readmeContent.match(/Stable tag:\s*([\\w.-]+)/i);
            versions['readme.txt stable tag'] = stableMatch ? stableMatch[1] : null;
        }

        const blockJsonFiles = glob.sync(path.join(rootDir, 'src/blocks/**/block.json'));
        if (blockJsonFiles.length > 0) {
            blockJsonFiles.forEach(file => {
                try {
                    ...
                    warning(`Unable to read version from ${file}: ${err.message}`);
                    addResult('important', 'version', `Invalid block.json at ${file}`, 'warn');
                }
            });
        }

        const targetVersion = versions.VERSION || versions['package.json'];
        const allMatch = targetVersion && Object.values(versions).every(v => v === targetVersion);

        if (allMatch) {
            success(`Version consistency: ${targetVersion}`);
            addResult('critical', 'version', `All version files match: ${targetVersion}`, 'pass');
        ...
        addResult('critical', 'version', `Version check failed: ${err.message}`, 'fail');
...
function checkQualityGates() {
    ...
    if (formatResult.success || formatResult.output?.includes('All matched files')) {
        success('Formatting: PASSED');
        addResult('important', 'quality', 'Code properly formatted', 'pass');
    } else {
        warning('Formatting: needs attention');
        addResult('important', 'quality', 'Code formatting inconsistent', 'warn');
    }
...
function checkDocumentation() {
    ...
    if (fs.existsSync(readmePath)) {
        ...
        if (readmeContent.includes('{{version}}')) {
            ...
            addResult('important', 'docs', 'README.md has unreplaced variables', 'warn');
        }
    } else {
    ...
    if (fs.existsSync(pluginReadmePath)) {
        ...
        if (/Stable tag:\s*\{\{version\}\}/.test(pluginReadmeContent)) {
            ...
            addResult('important', 'docs', 'readme.txt stable tag not updated', 'warn');
        } else {
...
function testPluginGeneration() {
    ...
    if (!fs.existsSync(configPath)) {
        ...
        addResult('important', 'generation', 'plugin-config.json missing', 'warn');
        return false;
    }
    ...
    if (result.success) {
        ...
        addResult('critical', 'generation', 'Generator validation passed', 'pass');
        return true;
    }
...
function runSecurityAudit() {
    ...
    if (result.success) {
        ...
        addResult('critical', 'security', 'No critical vulnerabilities', 'pass');
        return true;
    } else {
        ...
        if (output.includes('found 0 vulnerabilities')) {
            ...
            console.log(output);
            addResult('critical', 'security', 'High/critical vulnerabilities found', 'fail');
            return false;
        }
    }
}
...
function generateReport() {
    ...
    if (isReady) {
        ...
        console.log(`2. Create release branch: git checkout -b release/${version}`);
        console.log(`3. Commit: git commit -am "chore: prepare release v${version}"`);
