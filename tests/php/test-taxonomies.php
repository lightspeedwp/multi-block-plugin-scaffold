...
class Test_Taxonomies extends WP_UnitTestCase {
	...
	public function test_taxonomy_registered() {
		$this->assertTrue( taxonomy_exists( '{{slug}}_category' ) );
	}
	...
	public function test_taxonomy_associated_with_post_type() {
		...
		$this->assertContains( '{{slug}}_category', $taxonomies );
	}
	...
	public function test_taxonomy_is_hierarchical() {
		$taxonomy = get_taxonomy( '{{slug}}_category' );
	...
	public function test_can_create_term() {
		$term = wp_insert_term( 'Test Category', '{{slug}}_category' );
	...
	public function test_can_assign_term_to_post() {
		...
		$term = wp_insert_term( 'Assigned Category', '{{slug}}_category' );

		$result = wp_set_object_terms( $post_id, $term['term_id'], '{{slug}}_category' );

		$this->assertIsArray( $result );
		$this->assertContains( $term['term_id'], $result );

		$post_terms = wp_get_object_terms( $post_id, '{{slug}}_category', array( 'fields' => 'ids' ) );
