/**
 * {{name}} Collection Block - Editor Component
 *
 * @package {{namespace}}
 */
...
export default function Edit( { attributes, setAttributes } ) {
	...
	const posts = useSelect(
		( select ) => {
			...
			return select( 'core' ).getEntityRecords( 'postType', '{{slug}}', queryArgs );
		},
		...
		<>
				...
				<PanelBody title={ __( 'Layout', '{{textdomain}}' ) }>
					<SelectControl
						...
						options={ [
							...
							{ label: __( 'List', '{{textdomain}}' ), value: 'list' },
			...
			<div { ...blockProps }>
				...
				{ posts && posts.length === 0 && (
					<p>{ __( 'No {{name_plural_lower}} found.', '{{textdomain}}' ) }</p>
				) }

				{ posts && posts.length > 0 && (
					<div className="wp-block-{{namespace}}-{{slug}}-collection__items" style={ gridStyle }>
						{ posts.map( ( post ) => (
							<article key={ post.id } className="wp-block-{{namespace}}-{{slug}}-collection__item">
