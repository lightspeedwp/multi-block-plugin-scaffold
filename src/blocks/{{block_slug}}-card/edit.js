...
export default function Edit({ attributes, setAttributes, context }) {
	...
	const blockProps = useBlockProps({
		className: 'wp-block-{{namespace}}-{{block_slug}}-card',
	});
		...
		<>
			...
			<div {...blockProps}>
				{displayFeaturedImage && featuredMedia && (
					<div className="wp-block-{{namespace}}-{{block_slug}}-card__image">
						<img
				...
				<div className="wp-block-{{namespace}}-{{block_slug}}-card__content">
					{displayTitle && post && (
						<h3 className="wp-block-{{namespace}}-{{block_slug}}-card__title">
							{post.title?.rendered ||
					...
					{displaySubtitle && (
						<p className="wp-block-{{namespace}}-{{block_slug}}-card__subtitle">
					...
					{displayExcerpt && post && (
						<div
							className="wp-block-{{namespace}}-{{block_slug}}-card__excerpt"
					...
					{displayMeta && post && (
						<div className="wp-block-{{namespace}}-{{block_slug}}-card__meta">
							<span className="wp-block-{{namespace}}-{{block_slug}}-card__date">
				...
				{!post && (
					<p className="wp-block-{{namespace}}-{{block_slug}}-card__placeholder">
