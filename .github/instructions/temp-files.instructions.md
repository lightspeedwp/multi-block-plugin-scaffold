...
You are a temporary artefact gatekeeper. Follow our repository rules to manage `tmp/` content safely. Avoid committing temporary files, storing long-lived assets here, or adding build outputs outside reproducible processes. **All scripts and agents must use the root-level `tmp/` and `logs/` directories, not subfolders like `scripts/dry-run/tmp` or `scripts/dry-run/logs`.**
...
- Test logs: `logs/test-unit-2025-12-11T10-30-00.log`
...
| `logs/` (root) | Volatile or debug logs. | `test-unit-2025-12-11T10-30-00.log`, `dry-run-2025-12-11T10-30-00.log` | Manual or script-based. |
...
Any automated process, script, or AI agent that uses the `tmp/` or `logs/` directories must adhere to the following rules. Do not use `scripts/dry-run/tmp` or `scripts/dry-run/logs`.
...
**CRITICAL**: Do not ever use `git add --force tmp/some-file.log` or add files from `scripts/dry-run/tmp` or `scripts/dry-run/logs` to bypass these rules. All temporary and log files must be in the root-level `tmp/` and `logs/` directories.
...
**As of December 2025, all scripts and agents must use the root-level `tmp/` and `logs/` directories.**

- Do **not** use `scripts/dry-run/tmp` or `scripts/dry-run/logs`.
- Migrate any script or test that writes to those subfolders to use the root-level `tmp/` and `logs/` instead.
- This ensures consistency, discoverability, and proper cleanup.
