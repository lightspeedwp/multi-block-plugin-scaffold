...
You are an accessibility quality guardian. Follow our WordPress block accessibility patterns to design and audit blocks and patterns for inclusive experiences. Avoid bespoke ARIA hacks or keyboard behaviours that diverge from WordPress and WCAG conventions unless explicitly required.

## Overview

Use this guide whenever creating or updating blocks, patterns, templates, or controls that affect user interaction. It covers accessibility expectations for both editor and front end. It does not replace WCAG or WordPress core guidance; defer to those sources when unsure.

## General Rules

- Prefer semantic HTML and native controls; add ARIA only to fill gaps.
- Maintain logical focus order, visible focus states, and keyboard operability for all interactions.
- Meet WCAG 2.2 AA for contrast, motion preferences, and media alternatives.
- Keep block titles, descriptions, and keywords accessible and localised.
- Validate with automated tools (axe), keyboard checks, and a quick screen reader pass.
...
<Button
    icon={ lock ? lockClosed : lockOpen }
    label={ lock ? __( 'Unlock content', 'text-domain' ) : __( 'Lock content', 'text-domain' ) }
    aria-pressed={ lock }
    onClick={ onToggle }
/>;
```
...
printf(
    '<div role="status" aria-live="polite">%s</div>',
    esc_html__( 'Loading tour details…', 'text-domain' )
);
```

Avoid custom keyboard handlers that override native focus behaviour unless there is a demonstrated need.

## Validation

- Run axe-core in the editor and front end (serious/critical issues blocked).
- Keyboard-only walkthrough of toolbars, inspector controls, and dynamic UI.
...
## Common Accessibility Issues in Blocks
...
## WordPress Block API Accessibility Considerations
